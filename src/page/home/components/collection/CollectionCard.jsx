...
const CollectionCard = ({ chart }) => {
  ...
  return (
    <article onClick={handleClick} className="flex gap-5 group relative cursor-pointer border-b border-t px-3 py-4">
      ...
      <div className="flex flex-1 rounded overflow-hidden brightness-95 grayscale-10">
        <img src={chart.cover_url} alt={chart.title} className="aspect-video size-full object-cover" />
      </div>

      <div className="flex flex-2 min-w-0 flex-col  justify-between">
        <div className="flex justify-between items-start gap-2">
          <div className="flex flex-col gap-1 text-left">
            <h3 className="max-w-2xl text-3xl font-semibold leading-tight tracking-tight ">{chart.title}</h3>
            <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground text-balance ">{chart.description}</p>
          </div>
        ...
        <div className="flex justify-between items-center gap-5">
          ...
          <Toggle aria-label="Toggle like" size="sm" variant="outline" onClick={(e) => e.stopPropagation()}>
