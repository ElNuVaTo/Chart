...
const PagesVisualize = () => {
  ...
  useEffect(() => {
    const loadChart = async () => {
      const { data, error } = await supabase.from("chart").select("*").eq("id", id).eq("user_id", user).single();
  ...
  if (!chart) {
    return <div className="flex min-h-screen items-center justify-center">Chart no encontrado</div>;
  }
      ...
      {chart.levels.map((level, levelIndex) => (
        <section key={levelIndex} className="relative mt-24 pb-15">
          ...
          <div className="mx-auto mb-8 max-w-2xl text-center">
            <p className="text-sm leading-6 text-muted-foreground">{level.takeaway}</p>
          </div>
